    ...
    while intentos < MAX_INTENTOS:
        ...
        try:
            async with async_playwright() as p:
                ...
                # Si detecta el resultado esperado, toma captura y finaliza
                resultado_detectado = False
                # Detectar por texto clave en la página
                page_text = (await page.content()).lower()
                if "consulta personas" in page_text and "estado del conductor" in page_text:
                    await page.screenshot(path=abs_png, full_page=True)
                    resultado_detectado = True
                elif await page.locator("#consulta").count() > 0:
                ...
                if resultado_detectado:
                    mensaje = "Consulta exitosa, resultado detectado."
                    score = 0
                    ...
                    try:
                        ...
                        pass
                    return
                # Si no detecta, sigue con el flujo anterior
                try:
                        ...
                        if await page.locator("#consulta tbody tr").count() > 0:
                            ...
                            if await page.locator("#success").count() > 0:
                                ...
                                if "no se ha encontrado" in html_low or "sin registro" in html_low or "no se encuentra" in html_low:
                                    mensaje = "La persona no se encuentra activa o no está registrada."
                                    score = 10
                                    # Guardar la captura de la pantalla de error
                                    try:
                                        await page.screenshot(path=abs_png, full_page=True)
                                    except Exception:
                                        pass
